...
function seedExercises(){
  ...
  add("ex_rdl_bb","Romanian Deadlift (BB)",["HAMSTRINGS","GLUTES"],true,"barbell");
...
// --- LOGIC HELPER ---
function exById(id){ return state.exercises.find(e=>e.id===id) || {id, name:"(Unknown Exercise)", muscles:[]}; }
...
function exerciseCard(slot, ui, slotIndex, totalSlots){
  ...
  const showSwap = ui.swapModalFor === slot.id;
  
  // Get similar exercises for swap (same primary muscle)
  const similarExercises = state.exercises
  ...
  return html`<div class="exerciseCard" style="margin-bottom:12px; border:${isSkipped?'1px solid var(--warn)':'1px solid var(--line)'}">
    ...
    <div class="exBody" style="${isCollapsed ? 'display:none' : 'display:block'}">
      ...
      <div class="row" style="margin-top:8px">
        <button class="btn secondary" data-open-swap="${slot.id}">Swap Exercise</button>
        <button class="btn danger" data-remove-slot="${slot.id}">Remove</button>
      </div>
      ${showSwap ? `
        <div class="modal">
          <div class="h2">Swap to:</div>
          <select class="input" id="swapExSelect_${slot.id}">
            ${similarExercises.map(e => `<option value="${e.id}">${escapeHtml(e.name)}</option>`).join("")}
          </select>
          <div class="row" style="margin-top:10px">
            <button class="btn ok" data-confirm-swap="${slot.id}">Confirm Swap</button>
            <button class="btn secondary" data-cancel-swap="${slot.id}">Cancel</button>
          </div>
        </div>
      ` : ""}
...
document.addEventListener("click", e => {
  ...
  if(ui) {
    ...
    if(ds.openSwap) { ui.swapModalFor = ds.openSwap; render(); return; }
    if(ds.cancelSwap) { ui.swapModalFor = null; render(); return; }
    if(ds.confirmSwap) {
      const slotId = ds.confirmSwap;
      const selectEl = document.getElementById(`swapExSelect_${slotId}`);
      if(selectEl && selectEl.value) {
        const slot = state.slots.find(s => s.id === slotId);
        if(slot) {
          slot.exId = selectEl.value;
          // Reset suggested load for new exercise
          slot.suggestedLoad = 0;
          saveState(state);
          // Reset draft for this slot
          delete ui.setDrafts[slotId];
          ui.swapModalFor = null;
          setStatus("Exercise swapped.");
          render();
        }
      }
      return;
    ...
    if(ds.removeSlot) {
      if(confirm("Remove this exercise from the workout?")) {
        const slotId = ds.removeSlot;
        state.slots = state.slots.filter(s => s.id !== slotId);
        saveState(state);
        delete ui.setDrafts[slotId];
        delete ui.skips[slotId];
